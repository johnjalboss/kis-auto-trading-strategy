...
def _guess_exchange_for_symbol(symbol: str) -> str:
    ...
    NYSE_KNOWN = {
        ...
        'PM', 'NEE', 'UBER', 'PLTR', 'SQ', 'SNAP', 'DKNG', 'NIO',
    }
...
def get_new_highs_lows(exchange: str = "NAS", sort: str = "1",
    ...
    auth = get_auth()
    _rate_limit()
    headers = auth.get_headers(tr_id="HHDFS76950600")
    params = {
        "EXCD": exchange,
        "GUBN": sort,  # 1=신고가, 2=신저가
        "BYMD": "", "MODP": "0",
    }
    
    try:
        resp = requests.get(
            f"{config.BASE_URL}/uapi/overseas-price/v1/quotations/inquire-new-high-low",
            headers=headers, params=params, timeout=10
        )
        resp.raise_for_status()
        data = resp.json()
