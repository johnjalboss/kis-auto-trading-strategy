...
class PreMarketGapSniper:
    ...
    def format_telegram_card(self, top_n: int = 5) -> str:
        ...
        lines.append("━━━━━━━━━━━━━━━━━━━")
        lines.append(
            "📖 <b>[특이 갭 레이더 초보자 3초 이해 가이드]</b>\n"
            "• 🔥 <b>급등 특이 갭 (+2.5% 이상)</b>: 밤사이에 호재/실적 서프라이즈가 터져 아침부터 기관 매수세가 강하게 쏠리는 종목 ➔ <b>[상승 돌파 매수 후보]</b>\n"
            "• 📉 <b>급락 특이 갭 (-2.5% 이하)</b>: 악재로 투매가 나온 종목으로, 무작정 사지 않고 바닥을 다진 뒤 턴어라운드 반등이 확인될 때까지 안전하게 감시"
        )
        return "\n".join(lines)
