...
class MacroAnalyzer:
    ...
    def analyze(self) -> MacroState:
        ...
        # [신규] VIX 급등 감지 (CRASH_PROTOCOL)
        crash_protocol = False
        vix_proxy = self._cache.get("vix")
        if vix_proxy is not None and len(vix_proxy) >= 2:
            vix_change = (vix_proxy.iloc[-1] / vix_proxy.iloc[-2] - 1) * 100
            if vix_change > 15.0: # 하루 15% 이상 폭등 시
                crash_protocol = True
                triggers.append(f"VIX_SPIKE:{vix_change:+.1f}%")
                logger.warning("CRASH_PROTOCOL ACTIVATED: Extreme volatility spike detected!")
        ...
        # VIX 폭등 시 비중 추가 절반 감축
        if crash_protocol:
            regime = MarketRegime.RISK_OFF # Force risk off
            betting *= 0.5
            total_score = min(total_score, -60) # Force highly negative score
