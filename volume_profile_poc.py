...
class VolumeProfilePOCEngine:
    ...
    def analyze(self, df: pd.DataFrame, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Check if current price is just bouncing off POC (0.0% to +2.5% above POC)
            if 0.0 <= dist_from_poc <= 2.5:
                # Check recent 3-day momentum is positive
                if len(close) >= 4 and close[-1] > close[-3]:
