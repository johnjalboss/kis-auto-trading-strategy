...
def get_market_regime():
    ...
    try:
        ...
        if curr_qqq > ma20_qqq and curr_spy > ma20_spy:
            ...
            # 장기 지지선 아래에 있더라도, 최근 5일간 강력하게 반등 중이면 CAUTION(회복세)으로 완화
            if ret_5d_qqq > 1.0 or ret_5d_spy > 1.0:
                status = "🟡 RECOVERING (낙폭 과대 후 반등 국면)"
                desc = "지수가 장기 지지선(MA50) 아래에 있으나, 최근 강력한 단기 반등 흐름이 관찰됩니다. 보수적인 분할 매수 진입이 유효합니다."
                color = "#f0b429"
            else:
                status = "🔴 RISK (현금 확보 / 매수 금지)"
                desc = "지수가 장기 하락 지지선(MA50)마저 이탈하고 단기 반등 모멘텀도 없는 침체장입니다. 신규 매수를 자제하고 리스크를 관리하십시오."
                color = "#ff3b5c"
        else:
