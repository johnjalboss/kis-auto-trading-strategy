...
class CompositeSignalEngine:
    ...
    WEIGHTS = {
        ...
        'technical': 0.35,
        'fundamental': 0.10,
        'smart_money': 0.20,    # Reduced from 0.25 to make room for sentiment
        'sentiment': 0.15,      # Increased from 0.10 for contrarian/early entry
        'risk': 0.10
    }
    ...
    def _run_analyzers_async(self, category_key: str, df: pd.DataFrame, **kwargs) -> tuple[int, list]:
        ...
        for analyzer in self.analyzers[category_key]:
            ...
            if cache_key in self._cache:
                ...
                expiry = CACHE_SHORT if analyzer.is_symbol_dependent else CACHE_LONG
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
                ...
                for future in done:
                    ...
                    try:
                        ...
                        # Update cache
                        cache_key = (analyzer.name, symbol if analyzer.is_symbol_dependent else 'GLOBAL')
    ...
    def analyze(self, symbol: str, df: Optional[pd.DataFrame] = None) -> CompositeSignal:
        ...
        macro = self._calculate_macro_score(df, symbol)
        technical = self._calculate_technical_score(df, symbol)
        fundamental = self._calculate_fundamental_score(df, symbol)
        smart_money = self._calculate_smart_money_score(df, symbol)
        sentiment = self._calculate_sentiment_score(df, symbol)
        risk = self._calculate_risk_score(df, symbol)
    ...
    def _calculate_macro_score(self, df: pd.DataFrame, symbol: str) -> CategoryScore:
        """Calculate macro environment score"""
        score = 20
        signals = []
        
        # Async run all MACRO modules (e.g. regime_detector, fed_watch)
        async_score, async_signals = self._run_analyzers_async('macro', df, symbol=symbol)
        score += async_score
        signals.extend(async_signals)

        # Check for Inverse ETF adjustment
        import config
        is_inverse = symbol in getattr(config, 'INVERSE_ETFS', set())
        
        # If market is bearish and ticker is inverse, negate the penalty
        # (Assuming async_score is negative during bear markets)
        if is_inverse:
            if async_score < 0:
                score += abs(async_score) * 2.5 # Give bonus for being inverse in bear market
                signals.append("BEAR_BULL_INVERSE")
            else:
                score -= 20 # Penalize inverse in bull market
                signals.append("BULL_BEAR_INVERSE")
            
        return CategoryScore("MACRO", min(100, max(-100, score)), self.WEIGHTS['macro'], signals)
    
    def _calculate_technical_score(self, df: pd.DataFrame, symbol: str) -> CategoryScore:
        ...
        async_score, async_signals = self._run_analyzers_async('technical', df, symbol=symbol)
        score += async_score
        signals.extend(async_signals)
        ...
        if is_inverse:
            ...
            if rsi > 72:
                score -= 60
                signals.append(f"INVERSE_EXHAUSTION:RSI_{rsi:.0f}")
            
            # 2. 시장 바닥 감지 (나스닥/S&P 과매도 시 인버스 매도)
            try:
                # QQQ를 시장 프록시로 사용
                market_df = self._fetch_data("QQQ")
                if market_df is not None:
                    market_rsi = self._calculate_rsi(market_df['Close'])
                    if market_rsi < 28:
                        score -= 80 # 강력한 매도 신호 유도
                        signals.append(f"MARKET_BOTTOM_DETECTION:QQQ_RSI_{market_rsi:.0f}")
            except Exception as e:
                logger.debug(f"Market bottom check failed: {e}")

        # 일반 RSI 로직
        if rsi < 30 and not is_inverse:
            score += 25
            signals.append(f"RSI_OVERSOLD:{rsi:.0f}")
        elif rsi > 70 and not is_inverse:
            score -= 25
        ...
        # Stocks >10% above SMA20 have historically had 5% win rate, -32% avg PnL
        dist_from_sma20 = (current - float(sma20)) / float(sma20)
        if dist_from_sma20 > 0.10:
            score -= 40
            signals.append(f"OVEREXTENDED:{dist_from_sma20:.1%}_vs_SMA20")
        elif dist_from_sma20 > 0.07:
            score -= 20
    ...
    def _calculate_fundamental_score(self, df: pd.DataFrame, symbol: str) -> CategoryScore:
        """Calculate fundamental score"""
        signals = []
        score = 0
        
        async_score, async_signals = self._run_analyzers_async('fundamental', df, symbol=symbol)
        score += async_score
        signals.extend(async_signals)
        ...
        # If a stock already ran up fast, we likely missed the move
    ...
    def _calculate_smart_money_score(self, df: pd.DataFrame, symbol: str) -> CategoryScore:
        """Calculate smart money score"""
        signals = []
        score = 0
        
        # Async run all SMART_MONEY modules (options_flow, institutional, etc)
        async_score, async_signals = self._run_analyzers_async('smart_money', df, symbol=symbol)
        score += async_score
        signals.extend(async_signals)
    ...
    def _calculate_sentiment_score(self, df: pd.DataFrame, symbol: str) -> CategoryScore:
        """Calculate sentiment score"""
        signals = []
        score = 0
        
        async_score, async_signals = self._run_analyzers_async('sentiment', df, symbol=symbol)
        score += async_score
        signals.extend(async_signals)
    ...
    def _calculate_risk_score(self, df: pd.DataFrame, symbol: str) -> CategoryScore:
        """Calculate risk score"""
        signals = []
        score = 0
        
        async_score, async_signals = self._run_analyzers_async('risk', df, symbol=symbol)
        score += async_score
        signals.extend(async_signals)
        ...
        high_50d = close.tail(50).max()
        drawdown = (current - high_50d) / high_50d
    ...
    def _generate_summary(self, action: ActionType, score: int, confidence: int,
        ...
        emoji = {
            ...
            ActionType.WEAK_BUY: "👍",
            ActionType.HOLD: "⏸️",
            ActionType.WEAK_SELL: "👎",
            ActionType.SELL: "📉",
            ActionType.STRONG_SELL: "🔻"
        }
        
        summary = f"{emoji.get(action, '')} {action.value} (Score: {score:+d}, Confidence: {confidence}%)\n"
        
        if bullish:
            summary += f"✅ Bullish: {', '.join(bullish[:3])}\n"
        if bearish:
            summary += f"❌ Bearish: {', '.join(bearish[:3])}\n"
        if warnings:
            summary += f"⚠️ Warnings: {', '.join(warnings)}"
...
if __name__ == "__main__":
    ...
    for symbol in ["AAPL", "NVDA", "TSLA"]:
        ...
        print(f"  🗣️ Sentiment:   {result.sentiment_score.score:+4d} | {result.sentiment_score.signals}")
        print(f"  ⚠️ Risk:        {result.risk_score.score:+4d} | {result.risk_score.signals}")
        print()
