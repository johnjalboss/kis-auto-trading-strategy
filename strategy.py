...
class StrategyEngine:
    ...
    def _calc_entry_confidence(self, ind: IndicatorSummary, 
                               ...
                               comp_signal, symbol: str = None) -> Tuple[int, List[str], float]:
        """Calculate entry confidence score using the Unified Quantitative Multi-Factor Engine.
        
        Evaluates 5 orthogonal factor pillars (Trend, Institutional, Microstructure, Macro, Catalyst)
        with continuous nonlinear activations and asymmetric tail risk damping.
        """
        try:
            from unified_quant_scoring_engine import get_quant_scoring_engine
            final_score, breakdown, raw_score, pillars = get_quant_scoring_engine().compute_composite_score(
                symbol=symbol,
                df=df,
                indicators=ind,
                macro_state=macro_score,
                strategy_context=self
            )
            return final_score, breakdown, float(raw_score)
        except Exception as e:
            logger.error("Unified quant scoring failed for {}: {}. Falling back to baseline 50.", symbol, e)
            return 50, ["• Quant 엔진 기본값 (50점)"], 50.0
