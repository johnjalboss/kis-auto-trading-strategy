...
class CompositeSignalEngine:
    ...
    def _generate_summary(self, action: ActionType, score: int, confidence: int,
        ...
        emoji = {
            ...
            ActionType.BUY: "?뱢",
            ActionType.WEAK_BUY: "?몟",
            ActionType.HOLD: "?몌툘",
            ActionType.WEAK_SELL: "?몠",
            ActionType.SELL: "?뱣",
            ActionType.STRONG_SELL: "?뵽"
        }
        ...
        if warnings:
            summary += f"?좑툘 Warnings: {', '.join(warnings)}"
...
if __name__ == "__main__":
    ...
    for symbol in ["AAPL", "NVDA", "TSLA"]:
        ...
        print(f"  ?뱤 Macro:       {result.macro_score.score:+4d} | {result.macro_score.signals}")
        print(f"  ?뱢 Technical:   {result.technical_score.score:+4d} | {result.technical_score.signals}")
        print(f"  ?뮥 Fundamental: {result.fundamental_score.score:+4d} | {result.fundamental_score.signals}")
        print(f"  ?룱 Smart Money: {result.smart_money_score.score:+4d} | {result.smart_money_score.signals}")
        print(f"  ?뿣截?Sentiment:   {result.sentiment_score.score:+4d} | {result.sentiment_score.signals}")
        print(f"  ?좑툘 Risk:        {result.risk_score.score:+4d} | {result.risk_score.signals}")
        print()
